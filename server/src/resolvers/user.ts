import { checkPassword, generateAccessToken, isAuthenticated } from "../helpers/auth";
import { getAllUsers, getUserByEmail, getUserByUsername, updateUserPasswordByUsername } from "../helpers/users";
import { createNewUser, deleteUserById } from "../helpers/users";

export const resolveCreateUser = async (_: any, { firstName, lastName, email, username, password }: any) => {
...
export const resolveDeleteUser = async (_: any, { id }: any, { authToken }: any) => {
...
export const resolveLoginWithEmail = async (_: any, { email, password }: any) => {
...
export const resolveUpdatePassword = async (_: any, { username, oldPassword, newPassword }: any, { authToken }: any) => {
    isAuthenticated(authToken);
